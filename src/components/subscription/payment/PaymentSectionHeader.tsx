  ...
  return (
    <CardHeader className="bg-gradient-to-br from-primary/10 via-primary/5 to-transparent pb-8 border-b border-primary/10">
      <div className="flex items-center gap-3.5 mb-4">
        <div className="bg-primary/10 p-2.5 rounded-full shadow-sm">
          <CreditCard className="h-5 w-5 text-primary" />
        </div>
        <CardTitle className="text-2xl font-bold tracking-wider">השלם את התשלום</CardTitle>
      </div>
      <CardDescription className="text-base mt-1.5 text-muted-foreground/90 leading-relaxed">
        אנא מלא את פרטי התשלום במערכת הסליקה המאובטחת
      </CardDescription>
      
      {/* Premium Plan Summary Box with refined design */}
      <div className="mt-8 rounded-xl overflow-hidden shadow-md border border-primary/15 bg-card">
        {/* Header with subtle gradient background */}
        <div className="bg-gradient-to-r from-primary/15 to-transparent p-6 border-b border-primary/10">
          <div className="flex items-center justify-between">
            <div className="flex flex-col">
              <h3 className="text-xl font-bold tracking-wide text-foreground">{planDetails.name}</h3>
              {isMonthlyPlan && (
                <div className="flex items-center gap-2 mt-3">
                  <Badge variant="secondary" className="bg-green-500/10 text-green-600 dark:text-green-400 border border-green-500/15 px-3 py-1 flex items-center gap-1.5">
            ...
            <div className="bg-background/95 backdrop-blur-sm border border-primary/10 p-4 rounded-xl shadow-sm transform hover:-translate-y-0.5 transition-transform">
        ...
        <div className="bg-background/50 p-6">
          ...
          <div className="flex items-center gap-2 mt-5 bg-primary/5 px-4 py-3 rounded-full w-fit border border-primary/15">
            {isMonthlyPlan ? (
              <>
...
const PlanFeature = ({ text }: { text: string }) => (
  <div className="flex items-center bg-background/90 p-3 rounded-lg border border-primary/10 shadow-sm hover:border-primary/20 transition-colors">
