...
const DigitalContractForm: React.FC<DigitalContractFormProps> = ({
  ...
  const handleSign = async () => {
    ...
    try {
      ...
      if (user) {
        // Call the izidoc-sign edge function to record the signature
        const response = await supabase.functions.invoke('izidoc-sign', {
          body: {
            userId: user.id,
            planId,
            fullName: userDetails.fullName,
      ...
      // Create a download link for the user to save the contract
      downloadContract();
  ...
  // Allow user to download the contract
  ...
  return (
    <Card className="max-w-3xl mx-auto glass-card-2025" dir="rtl">
      ...
      <CardContent className="space-y-6">
        <div className="relative">
          ...
          >
            ...
            <p><strong>6.1 תקופת הרישיון</strong> – תקופת הרישיון תיקבע בהתאם לסוג ולחבילת הרישיון שרכש המשתמש. ניתן לבטל את המנוי בכל עת באמצעות פנייה לשירות הלקוחות.</p>
            ...
            <p><strong>11.7 כותרות</strong> – כותרות הסעיפים ופרקי המשנה בהסכם זה ניתנו לשם הנוחות וההתמצאות בלבד, ואין לייחס להן משמעות פרשנית. כותרת סעיף אינה בהכרח ממצה את מלוא תוכן הסעיף, ואין להשתמש בה לשם פרשנות ההסכם.</p>
            ...
            <p><strong>12.5</strong> אחריות החלטות והשקעות. המשתמש לבדו נושא באחריות מלאה ובלעדית לכל החלטה, פעולה או אי-פעולה הננקטת על ידו או באמצעות התוכנה בחשבון ההשקעות שלו. המשתמש מתחייב לקבלת החלטות השקעה באופן עצמאי, לאחר שקילה ובדיקה המתאימים לו, והוא מודע לכך שכל שימוש בתוכנה הינו כלי עזר בלבד שאינו מחליף שיקול דעת עצמאי או ייעוץ מקצועי מותאם אישית. המשתמש מוותר בזאת על כל טענה כי הסתמך על החברה בקבלת החלטות השקעה. היה והמשתמש יבקש ייעוץ השקעות המתחשב בנסיבותיו האישיות, הוא אחראי לפנות ליועץ השקעות מוסמך מטעמו.</p>
            <p><strong>12.6</strong> שיפוי והגנה. המשתמש ישפה ויגן על החברה, מנהליה, עובדיה וכל מי מטעמה, מפני כל טענה, תביעה, הפסד או נזק (לרבות הוצאות משפט סבירות) הנובעים מטענה של צד שלישי או של המשתמש עצמו שהחברה פעלה כיועץ השקעות או מנהל השקעות כלפי המשתמש, בניגוד למוסכם לעיל. סעיף שיפוי זה בא להבהיר את כוונת הצדדים כי האחריות על פעילות ההשקעה היא של המשתמש, וכי החברה נוקטת באמצעים חוזיים להתגונן מכל תביעה שתייחס לה תפקיד של מנהל או יועץ בלתי מורשה.</p>
          ...
          {!scrolledToBottom && (
            <div className="absolute bottom-0 left-0 right-0 flex justify-center p-2 bg-gradient-to-t from-background to-transparent">
              <div className="flex flex-col items-center">
                <span className="text-xs text-muted-foreground mb-1">גלול למטה לקריאת כל ההסכם</span>
                <ArrowDown className="animate-bounce h-4 w-4 text-primary" />
              </div>
            </div>
          )}
        </div>
        
        <div className="space-y-4 border rounded-md p-4 bg-card/30">
          <h3 className="font-medium text-lg">פרטי המשתמש</h3>
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <h3 className="font-medium text-lg">חתימה דיגיטלית</h3>
            <p className="text-sm text-muted-foreground">
              אנא צייר את חתימתך בשדה המיועד לכך למטה
            </p>
            <SignaturePad onChange={setSignature} value={signature} height={150} />
          </div>
          ...
          <div className="space-y-4 mt-6">
            ...
            {!scrolledToBottom && (
              <div className="text-sm text-amber-600 dark:text-amber-400">
                * יש לקרוא את כל ההסכם ולגלול עד לסופו לפני שניתן לסמן את תיבות הסימון
              </div>
            )}
          </div>
        </div>
      ...
      <CardFooter className="flex gap-4">
        ...
        <Button
          onClick={handleSign}
          disabled={!signature || !agreedToTerms || !agreedToPrivacy || isSigningInProgress || !scrolledToBottom || !userDetails.fullName}
