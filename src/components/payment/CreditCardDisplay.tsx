...
const CreditCardDisplay: React.FC<CreditCardDisplayProps> = ({
  ...
  const ccIconRef = useRef<HTMLDivElement>(null);
  const ccSingleRef = useRef<HTMLDivElement>(null);
  ...
  // Function to change card color based on the card type
  const swapColor = (basecolor: string) => {
    if (creditCardRef.current) {
      const lightColorElements = creditCardRef.current.querySelectorAll('.lightcolor');
      const darkColorElements = creditCardRef.current.querySelectorAll('.darkcolor');
      
      lightColorElements.forEach(element => {
        element.classList.forEach(cls => {
          if (cls.startsWith('light') && cls !== 'lightcolor') {
      ...
      darkColorElements.forEach(element => {
        element.classList.forEach(cls => {
          if (cls.startsWith('dark') && cls !== 'darkcolor') {
            element.classList.remove(cls);
          }
        });
        element.classList.add(basecolor + 'dark');
      });
    }
  };

  // Detect card type and update card icon
  useEffect(() => {
    const detectCardType = (number: string) => {
      ...
      // American Express
      if (/^3[47]/.test(cardNum)) {
        return { type: 'american express', color: 'green' };
      }
      // Visa
      else if (/^4/.test(cardNum)) {
        return { type: 'visa', color: 'lime' };
      }
      // Mastercard
      else if (/^(5[1-5]|2[2-7])/.test(cardNum)) {
        return { type: 'mastercard', color: 'lightblue' };
      }
      // Discover
      ...
      // JCB
        ...
        return { type: 'jcb', color: 'red' };
      }
      // Diners Club
      else if (/^3(?:0[0-5]|[68])/.test(cardNum)) {
        return { type: 'diners', color: 'orange' };
      }
      // Default
      else {
        return { type: '', color: 'grey' };
      }
    };

    if (cardNumber) {
      const { color } = detectCardType(cardNumber);
      swapColor(color);
    }
  }, [cardNumber]);

  const displayCardNumber = cardNumber.length > 0 ? cardNumber : '0123 4567 8910 1112';
  const displayName = cardholderName.length > 0 ? cardholderName : 'ישראל ישראלי';
  const displayExpiry = expiryDate.length > 0 ? expiryDate : 'MM/YY';
  const displayCVV = cvv.length > 0 ? cvv : '123';
    ...
    <div className={styles.container}>
      ...
      >
        <div className={styles.front}>
          <div ref={ccSingleRef} className={styles.ccsingle}></div>
          <svg version="1.1" id="cardfront" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink"
            ...
            <g id="Front">
              ...
              <text transform="matrix(1 0 0 1 60.106 295.0121)" id="svgnumber" className="st2 st3 st4">{displayCardNumber}</text>
              <text transform="matrix(1 0 0 1 54.1064 428.1723)" id="svgname" className="st2 st5 st6">{displayName}</text>
              ...
              <g>
                <text transform="matrix(1 0 0 1 574.4219 433.8095)" id="svgexpire" className="st2 st5 st9">{displayExpiry}</text>
        ...
        <div className={styles.back}>
          <svg version="1.1" id="cardback" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink"
            ...
            <g id="Back">
              ...
              <text transform="matrix(1 0 0 1 621.999 227.2734)" id="svgsecurity" className="st6 st7">{displayCVV}</text>
              <g className="st8">
              ...
              <text transform="matrix(1 0 0 1 59.5073 228.6099)" id="svgnameback" className="st12 st13">{displayName}</text>
            </g>
          </svg>
        </div>
      </div>
      <div ref={ccIconRef} className={styles.ccicon}></div>
    </div>
  );
};
