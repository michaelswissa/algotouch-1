...
serve(async (req) => {
  ...
  try {
    ...
    let lowProfileCode, sessionId, terminalNumber, timestamp, attempt, planType;
    ...
    logStep("CardCom status response details", { 
      operationResponse, dealResponse, transactionId, returnValue, description, operation, threeDSResult, token, tokenResponse, tokenExDate, attempt
    });
    ...
    const tokenCreatedSuccessfully =
      isTokenCreation && !!token && tokenResponse === '0';
    ...
    if (tokenCreatedSuccessfully) {
      ...
      return new Response(JSON.stringify({
        ...
        data: {
          token, tokenExDate, lowProfileCode, tokenResponse, cardOwnerEmail, cardOwnerPhone, cardOwnerName
        }
      }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    if (isTokenCreation && tokenResponse && tokenResponse !== '0') {
      return new Response(JSON.stringify({
        failed: true,
        success: false,
        message: description || 'יצירת האסימון נכשלה',
        data: {
          token, tokenExDate, tokenResponse, lowProfileCode
        }
      }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    if (isInitialProcessing || is3DSProcess) {
      return new Response(
        JSON.stringify({
          ...
          data: { operationResponse, dealResponse, description, is3DSProcess, operation, threeDSResult, attempt,
            cardcomResponse: { status: cardcomResponse.status, ok: cardcomResponse.ok }
          }
        }),
        {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
