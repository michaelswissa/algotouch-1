...
serve(async (req) => {
  ...
  try {
    ...
    if (isSuccessful && isMonthlyPlan && TokenInfo) {
      const tokenData = {
        ...
        try {
          await supabaseAdmin
            ...
            .insert({
              ...
              token_expiry: new Date(parseInt(tokenData.tokenExpiryDate.substring(0, 4)), 
                                   parseInt(tokenData.tokenExpiryDate.substring(4, 6)) - 1,
      ...
      if (paymentSession.user_id && paymentMethod?.token) {
        ...
        try {
          ...
          if (!existingToken || existingToken.length === 0) {
            const tokenExpiry = new Date(parseInt(tokenData.tokenExpiryDate.substring(0, 4)), 
                                       parseInt(tokenData.tokenExpiryDate.substring(4, 6)) - 1,
                                       1).toISOString().split('T')[0];
